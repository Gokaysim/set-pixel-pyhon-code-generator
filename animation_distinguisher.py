def get_animation():
    s = "\nint AnimationFrameDelay(short i)"
    s += "\n{"
    s += "\n\tshort animation_length_index = animation_name_length[i];"
    s += "\n\tchar current_animation[animation_length_index];"
    s += "\n\tstrcpy(current_animation, included_animations[i]);"
    s += "\n\tint animation_delay_time;"
    s += '\n\n\tif(strcmp(current_animation, "animefight") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 200000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "bavaria") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 500000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "birds") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 400000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "colorshift") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 2000000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "dove") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 300000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "ellipse") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 450000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "explosion") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 150000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "fadedrainbow") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 500000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "flames") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 500000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "feather") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 200000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "hexagons") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 500000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "infinity") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 150000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "inoutcircles") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 500000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "inovalogo") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 550000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "inovatext") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 450000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "inovalogoprep") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 400000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "loading") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 250000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "newyear") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 450000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "northernlights") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 400000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "pokemon") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 600000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "rainbowring") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 250000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "rainbowwave") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 350000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "shapes3d") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 250000;'
    s += '\n\t}'
    s += '\n\telse if(strcmp(current_animation, "spiralfullscreen") == 0)'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 350000;'
    s += '\n\t}'
    s += '\n\telse'
    s += '\n\t{'
    s += '\n\t\tanimation_delay_time = 2000000;'
    s += '\n\t}'
    s += "\n\n\treturn animation_delay_time;"
    s += "\n}\n"

    return s
